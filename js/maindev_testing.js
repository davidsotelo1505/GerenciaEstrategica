...
function mainLoop() {
    ...
    $("#clickcanvas").mousemove(function(e) {
        ...
        // aplica el dezplazamiento del puntero del mause
        tempX = tempX - 155;
        tempY = tempY - 80;
    ...
    $('#infoContent').append(
        "<button id='endTurnButton' style='position:absolute; left: 53px; top: 90px;'>Finalizar turno</button>"
    );
...
function greeting() {
    $('#alertArea').append(
        '<img src="img/greetings.png" width="575" height="95"/><br /><br /><b>Juego de estrategia es un juego de estrategia simple </b> en el que mueves tus unidades por un tablero tratando de eliminar al otro jugador. <br /> <br /> REGLAS <br /> <ul> <li> Para mover una unidad, haz clic en cualquier hexagono con uno de tus ejércitos en él, y haz clic en un hexagono de destino. <li> Debes dejar al menos una unidad en un hexagono ocupado, por lo que los hexagonos con solo una unidad no son elegibles para movimiento. <li> Cuando hayas terminado un movimiento, presione el botón Finalizar turno para permitir que su oponente se mueva. </ul> COMBATE <br /> Si el hexagono de destino está ocupado, la facción con el mayor número de ejercitos participantes gana el control del hexagono. <br /> <br /> REFUERZOS <br/> Al final de cada turno, cada hexagono ocupado genera un ejército adicional. <br /> <br /> GANAR <br /> Un jugador gana la ronda si el jugador contrario ya no tiene unidades para mover.'
    );

    $('#alertArea').dialog({
        title: "Bienvenido a juego de estrategia",
        draggable: false,
        position: [140, 55],
        minHeight: 100,
        minWidth: 600,
        ...
        buttons: {
            "Iniciar Juego": function() {
                $(this).dialog("close");
                gameBoardSetup();

                // Muestra recuadro turno de juegador 
                showMsgSide("<img src='img/gemRed.png' style='float:left; margin: 0 10px 0 0;'/><b>Turno jugador 1</b>");
...
function nextTurn() {
    if (playerIndex > 3) {
        ...
        // update player message
        showMsgSide("<img src='img/gemBlue.png' style='float:left; margin: 0 10px 0 0;'/><b>Turno jugador 2</b>");
...
function showHexStats(hexStat) {
    ...
    contextStatText.fillText(attVal + terrainData[terrainIndex][3] + " Reputacion", 60, 42);
    contextStatText.fillText(defVal + terrainData[terrainIndex][2] + " Fuerza", 60, 57);
